...
static const uint32_t THR = COM1 + 0; // Transmitter Holding Register
static const uint32_t IER = COM1 + 1; // Interrupt Enable Register
static const uint32_t DL_LO = COM1 + 0;   // Divisor Latch
static const uint32_t DL_HI = COM1 + 1;   // Divisor Latch
static const uint32_t FCR = COM1 + 2; // FIFO Control Register
static const uint32_t LCR = COM1 + 3; // Line Control Register
static const uint32_t LSR = COM1 + 5; // Line Status Register
...
inline void init();
inline void sendchr (char chr);
inline void sendstr (const char *str);
